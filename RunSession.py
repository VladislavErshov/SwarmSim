...
from src.system import MultiAgentSystem, LinearMAS
...
def linear_mpc():
    ...
    mas = LinearMAS(Q, R, P, 
                    n_agents, agent_dim, goal_state, state_gen=gen.uniform_cube, state_gen_args=[2, A, B])
    for sdx in range(n_steps):
        fstate = mas.get_full_state()
        system_centroid = mas.get_cluster_centroids()[0]
        if sdx % val_each == 0:
